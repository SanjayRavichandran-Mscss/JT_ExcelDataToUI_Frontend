...
const CreateRecords = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.supplier_number?.trim()) {
      result = result.filter(r => r.supplier_number?.toLowerCase().includes(filters.supplier_number.toLowerCase().trim()));
    }
    if (filters.supplier_name?.trim()) {
      result = result.filter(r => r.supplier_name?.toLowerCase().includes(filters.supplier_name.toLowerCase().trim()));
    }
  ...
  const isFormValid = () => {
    ...
    const traceabilityOk = traceabilityUniqueStatus.isUnique && !traceabilityUniqueStatus.checking;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 bg-white p-6 rounded-xl border shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={resetFilters} className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-black cursor-pointer">
              <RotateCcw size={16} /> Reset
            </button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
            {['tc_no', 'traceability_no', 'supplier_number', 'supplier_name', 'heat_no', 'size', 'material_grade'].map((key) => (
            ...
            <table className="w-full min-w-[1800px] text-left border-collapse">
              <thead>
                <tr className="bg-gray-100 border-b">
                  ...
                  <th className="py-4 px-6 font-bold uppercase text-sm text-gray-700">Supplier No.</th>
                  <th className="py-4 px-6 font-bold uppercase text-sm text-gray-700">Supplier Name</th>
                ...
                {loading ? (
                  ...
                  filteredRecords.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-4 px-6">{r.supplier_number || '-'}</td>
                      <td className="py-4 px-6 font-medium">{r.supplier_name || '-'}</td>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col border border-gray-200">
              <div className="flex justify-between items-center p-6 border-b bg-gray-50 rounded-t-2xl">
                ...
                <button onClick={closeModal} className="p-2 hover:bg-gray-200 rounded-full transition cursor-pointer">
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6">
                <div className="space-y-7">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Traceability No.</label>
                      <input
                        type="text"
                        ...
                        className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 transition-colors ${
                          traceabilityUniqueStatus.message ? 'border-red-500 bg-red-50' : 'border-gray-300'
                        }`}
                      />
                      {traceabilityUniqueStatus.checking && <p className="text-xs text-gray-500 mt-1">Checking...</p>}
                      {traceabilityUniqueStatus.message && (
                        <p className="text-xs text-red-600 font-medium mt-1">{traceabilityUniqueStatus.message}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-6 rounded-xl border">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Chemical Composition (%)</h3>

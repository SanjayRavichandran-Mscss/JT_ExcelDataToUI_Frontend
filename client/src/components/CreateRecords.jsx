...
//         // Calculate per-component counts
//         let validComp = 0;
//         let invalidComp = 0;
...
//             validComp += 11;
//           } else if (item.chemicalErrors) {
//             const errorCount = Object.keys(item.chemicalErrors).length;
//             invalidComp += errorCount;
//             validComp += 11 - errorCount;
//           }
//         });

//         setComponentCounts({ valid: validComp, invalid: invalidComp });

//         if (!data.allValid) {
//           setMessage({
//             text: `Found ${data.invalidCount || 0} invalid row(s). Fix before upload.`,
//             type: 'error'
//           });
//         } else {
//           setMessage({ text: 'All rows valid — ready to upload', type: 'success' });
//         }
//       } else {
//         setMessage({ text: data.message || 'Validation failed', type: 'error' });
//       }
//     } catch (err) {
//       setMessage({ text: 'Network error during validation', type: 'error' });
//     } finally {
...
//                 <h2 className="text-2xl font-bold">{editingRecord ? 'Edit Record' : 'Add New Record'}</h2>
//                 <button 
//                   onClick={closeModal} 
...
//                         className="w-full p-2 border rounded focus:ring-2 focus:ring-black cursor-text"
//                         required
//                       />
//                     </div>
...
//                   <div className="space-y-4">
//                     <h3 className="font-bold text-lg border-b pb-2">Chemical Composition (%)</h3>
                    
//                     <div className="grid grid-cols-2 gap-4">
//                       {['c','cr','ni','mo','mn','si','s','p','cu','fe','co'].map(elem => {
//                         const min = limits[`${elem}_min`];
//                         const max = limits[`${elem}_max`];
//                         const showHint = min && max;
                        
//                         return (
//                           <div key={elem}>
//                             <label className="block text-sm font-medium uppercase mb-1">{elem}</label>
//                             <input
//                               type="number"
//                               step="0.0001"
//                               name={elem}
...
//                               onChange={handleInputChange}
//                               className={`w-full p-2 border rounded focus:ring-2 focus:ring-black cursor-text ${
//                                 showHint ? 'border-blue-200' : ''
//                               }`}
//                               placeholder={showHint ? `${min} - ${max}` : ''}
//                               title={showHint ? `Allowed range: ${min} - ${max}` : ''}
//                             />
//                           </div>
//                         );
//                       })}
...
//                         <p className="font-medium text-blue-800">Allowed ranges for {formData.material_grade}:</p>
...
//                     type="button"
//                     onClick={closeModal}
//                     className="px-6 py-2 border rounded hover:bg-gray-50 cursor-pointer"
//                   >
//                     Cancel
//                   </button>
//                   <button
//                     type="submit"
//                     disabled={loading}
//                     className="px-6 py-2 bg-black text-white rounded hover:bg-gray-800 disabled:bg-gray-400 flex items-center gap-2 cursor-pointer disabled:cursor-not-allowed"
//                   >
...
//                 <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Bulk Upload – Excel Validation</h2>
//                 <button
//                   onClick={() => {
//                     setIsBulkModalOpen(false);
//                     setBulkFile(null);
...
//                       className={`px-10 py-4 rounded-xl text-white font-medium text-lg cursor-pointer ${
//                         bulkValidating ? 'bg-gray-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700'
//                       } transition shadow-md`}
//                     >
//                       {bulkValidating ? (
//                         <span className="flex items-center gap-3">
//                           <Loader2 className="animate-spin" size={24} /> Validating file...
//                         </span>
//                       ) : (
//                         'Choose Excel File'
//                       )}
...
//                   {/* Validation Results – Table-like View */}
...
//                             <span className="font-bold text-gray-900">{validationSummary.total}</span>
//                           </div>
//                           <div>
//                             <span className="text-gray-600">Valid rows:</span>{' '}
//                             <span className="font-bold text-green-700">{validationSummary.valid}</span>
//                           </div>
//                           <div>
//                             <span className="text-gray-600">Invalid rows:</span>{' '}
//                             <span className="font-bold text-red-700">{validationSummary.invalid}</span>
//                           </div>
//                           <div>
//                             <span className="text-gray-600">Valid components:</span>{' '}
//                             <span className="font-bold text-green-700">{componentCounts.valid}</span>
//                           </div>
//                           <div>
//                             <span className="text-gray-600">Invalid components:</span>{' '}
//                             <span className="font-bold text-red-700">{componentCounts.invalid}</span>
//                           </div>
//                         </div>
//                       </div>

//                       {/* Results - card per row */}
...
//                                 All chemical components are within allowed ranges
//                               </div>
//                             ) : (
//                               <div className="overflow-x-auto">
...
//                                   <tbody className="divide-y">
//                                     {item.chemicalErrors ? (
//                                       Object.entries(item.chemicalErrors).map(([chemKey, errorMsg]) => {
//                                         const match = errorMsg.match(/([\d.]+) is outside allowed range \(([\d.-]+) – ([\d.-]+)\)/);
//                                         const value = match ? match[1] : '—';
//                                         const range = match ? `${match[2]} – ${match[3]}` : '—';

//                                         return (
//                                           <tr key={chemKey} className="hover:bg-gray-50">
//                                             <td className="px-4 py-3 font-medium uppercase">{chemKey}</td>
...
//                                     ) : (
//                                       <tr>
//                                         <td colSpan={4} className="px-4 py-6 text-center text-red-700">
...
//                     setComponentCounts({ valid: 0, invalid: 0 });
//                   }}
//                   className="px-8 py-3 text-gray-700 hover:text-black font-medium text-lg cursor-pointer"
//                 >
//                   Cancel
//                 </button>
...
//                   disabled={bulkUploading || !allRowsValid || !bulkFile || bulkValidating}
//                   className={`px-10 py-3 rounded-xl font-bold text-lg flex items-center gap-3 shadow-lg cursor-pointer ${
//                     allRowsValid && bulkFile && !bulkUploading && !bulkValidating

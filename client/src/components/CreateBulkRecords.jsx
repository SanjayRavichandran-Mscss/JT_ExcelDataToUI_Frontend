...
//         const invalidCount = processed.filter(r => r.message && r.message.trim() !== '').length;
//         setAllRowsValid(invalidCount === 0);

//         setValidationSummary({
//           total: data.totalRows || processed.length || 0,
//           valid: data.validCount || processed.length - invalidCount,
//           invalid: data.invalidCount || invalidCount
//         });
//       } else {
//         setMessage({
//           text: data.message || 'Validation failed – check file format or server logs',
//           type: 'error'
//         });
//       }
//     } catch (err) {
//       console.error('Validation network error:', err);
//       setMessage({ text: 'Failed to reach validation endpoint', type: 'error' });
//     } finally {
...
//       setMessage({ text: 'Cannot upload: file missing or has validation errors', type: 'error' });
//       return;
//     }
...
//         // Refresh parent list but DO NOT close modal
//         onRecordsAdded();

//         // Clear form so user can upload another file if desired
//         setBulkFile(null);
//         setValidationResults([]);
//         setAllRowsValid(false);
//         setValidationSummary({ total: 0, valid: 0, invalid: 0 });
//       } else {
//         setMessage({
//           text: data.message || 'Upload failed – check server response',
//           type: 'error'
//         });
//       }
//     } catch (err) {
...
//         const match = part.match(/([A-Za-z\s]+?)(?:\s+is)?\s+required/i);
//         if (match && match[1]) missing.push(match[1].trim());
//       } else if (part.includes('outside') || part.includes('out of range')) {
//         const match = part.match(/([A-Za-z]+):\s*([\d.]+)\s*(?:is )?outside.*\(([\d.-]+)\s*[-–—]\s*([\d.-]+)\)/i);
//         if (match) {
//           outOfRange.push({
//             comp: match[1].toUpperCase(),
//             value: match[2],
//             min: match[3],
//             max: match[4]
//           });
...
//                 <p className="text-3xl font-bold text-green-700">{validationSummary.valid}</p>
//               </div>
//               <div>
//                 <p className="text-gray-600">Invalid rows</p>
//                 <p className="text-3xl font-bold text-red-700">{validationSummary.invalid}</p>
//               </div>
//             </div>
//           </div>
...
//               return (
//                 <div key={idx} className="border rounded-lg overflow-hidden bg-red-50/30 shadow-sm">
//                   <div className="px-6 py-4 bg-red-50 flex justify-between items-center">
//                     <h4 className="font-bold text-lg text-red-900">Row {item.row || (idx + 2)}</h4>
